...
<div class="col-sm-12"><table id="example2" class="table table-bordered table-hover dataTable dtr-inline" aria-describedby="example2_info">
                ...
                @foreach($seos as $seo)
                <tr class="odd">
                    <form action="{{route('admin.area.update', $seo->id)}}" method="post">
                        @csrf
                        @method('patch')
                    <td class="dtr-control sorting_1" tabindex="0">{{$seo->id}}</td>
                    <td>
                            <input type="text" class="form-control" name = 'name' placeholder="Enter ..." value="{{$seo->route}}">
                    </td>
                    <td>
                        <input type="text" class="form-control" name = 'price' placeholder="Enter ..." value="{{$seo->description}}">
                    </td>
                    <td>
                        <input type="text" class="form-control" name = 'discont' placeholder="Скидка в процентах" value="{{$seo->keywords}}">
                    </td>
                        <td><button type="submit" class="btn btn-primary">OK</button></td>
                    </form>
                    <td><form action="{{route('admin.area.delete', $seo->id)}}" method="post">
                            @csrf
                            @method('delete')
                ...
                <tr class="odd">
                    <form action="{{route('admin.area.store')}}" method="post">
                        @csrf
                        <td class="dtr-control sorting_1" tabindex="0">{{$seo->id + 1}}</td>
                        <td>
